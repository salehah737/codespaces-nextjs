...
import HondaPartScanner from '../../components/scanner/HondaPartScanner';
import ScrollReveal from '../../components/ScrollReveal';
import CTAButton from '../../components/CTAButton';
import { hondaModels } from '../../lib/honda-parts-data';

export default function HondaScannerPage() {
  ...
  const handleScanResult = (results) => {
    ...
    track('honda_part_scanned', { count: results.length });
  };

  const handleModelSelect = (modelId) => {
    setSelectedModel(hondaModels.find(m => m.id === modelId));
    track('honda_model_selected', { model: modelId });
  };
      ...
      <div className="container mx-auto px-4">
        <ScrollReveal>
          <h1 className="text-3xl font-bold text-center mb-2">
            {language === 'ms' ? 'Pengimbas Alat Ganti Honda' : 'Honda Parts Scanner'}
          </h1>
          <p className="text-gray-600 text-center mb-8">
            {language === 'ms' 
              ? 'Imbas alat ganti atau manual Honda untuk mendapatkan maklumat terperinci' 
              : 'Scan Honda parts or manuals to get detailed information'}
          </p>
        </ScrollReveal>
        ...
        <div className="max-w-3xl mx-auto">
          {!selectedModel ? (
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <h2 className="text-xl font-semibold mb-4">
                {language === 'ms' ? 'Pilih Model Honda' : 'Select Honda Model'}
              </h2>
          ...
          <div className="mt-8 bg-blue-50 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-2">
              {language === 'ms' ? 'Tentang Pengimbas Alat Ganti Honda' : 'About Honda Parts Scanner'}
            </h2>
            <p className="text-gray-700 mb-4">
              {language === 'ms' 
                ? 'Pengimbas ini direka untuk mengenal pasti alat ganti Honda Malaysia berdasarkan imej atau manual. Ia menyokong model-model berikut:' 
                : 'This scanner is designed to identify Honda Malaysia parts based on images or manuals. It supports the following models:'}
            </p>
            ...
            <p className="text-sm text-gray-600">
              {language === 'ms' 
                ? 'Data alat ganti diambil dari katalog rasmi Honda Malaysia.' 
                : 'Parts data is sourced from the official Honda Malaysia catalog.'}
            </p>
