...
import YamahaPartScanner from '../../components/scanner/YamahaPartScanner';
import ScrollReveal from '../../components/ScrollReveal';
import CTAButton from '../../components/CTAButton';
import { yamahaModels } from '../../lib/yamaha-parts-data';

export default function YamahaScannerPage() {
  ...
  const handleScanResult = (results) => {
    ...
    track('yamaha_part_scanned', { count: results.length });
  };

  const handleModelSelect = (modelId) => {
    setSelectedModel(yamahaModels.find(m => m.id === modelId));
    track('yamaha_model_selected', { model: modelId });
  };
      ...
      <div className="container mx-auto px-4">
        <ScrollReveal>
          <h1 className="text-3xl font-bold text-center mb-2">
            {language === 'ms' ? 'Pengimbas Alat Ganti Yamaha' : 'Yamaha Parts Scanner'}
          </h1>
          <p className="text-gray-600 text-center mb-8">
            {language === 'ms' 
              ? 'Imbas alat ganti atau manual Yamaha untuk mendapatkan maklumat terperinci' 
              : 'Scan Yamaha parts or manuals to get detailed information'}
          </p>
        </ScrollReveal>
        ...
        <div className="max-w-3xl mx-auto">
          {!selectedModel ? (
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <h2 className="text-xl font-semibold mb-4">
                {language === 'ms' ? 'Pilih Model Yamaha' : 'Select Yamaha Model'}
              </h2>
          ...
          <div className="mt-8 bg-blue-50 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-2">
              {language === 'ms' ? 'Tentang Pengimbas Alat Ganti Yamaha' : 'About Yamaha Parts Scanner'}
            </h2>
            <p className="text-gray-700 mb-4">
              {language === 'ms' 
                ? 'Pengimbas ini direka untuk mengenal pasti alat ganti Yamaha Malaysia berdasarkan imej atau manual. Ia menyokong model-model berikut:' 
                : 'This scanner is designed to identify Yamaha Malaysia parts based on images or manuals. It supports the following models:'}
            </p>
            ...
            <p className="text-sm text-gray-600">
              {language === 'ms' 
                ? 'Data alat ganti diambil dari katalog rasmi Yamaha Motor Malaysia.' 
                : 'Parts data is sourced from the official Yamaha Motor Malaysia catalog.'}
            </p>
            <p className="text-sm text-gray-600 mt-1">
              <a 
                href="https://www.yamaha-motor.com.my/products/" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-primary-600 hover:underline"
              >
                https://www.yamaha-motor.com.my/products/
              </a>
            </p>
          </div>
        </div>
