...
import PeroduaPartScanner from '../../components/scanner/PeroduaPartScanner';
import ScrollReveal from '../../components/ScrollReveal';
import CTAButton from '../../components/CTAButton';
import { peroduaModels } from '../../lib/perodua-parts-data';

export default function PeroduaScannerPage() {
  ...
  const handleScanResult = (results) => {
    ...
    track('perodua_part_scanned', { count: results.length });
  };

  const handleModelSelect = (modelId) => {
    setSelectedModel(peroduaModels.find(m => m.id === modelId));
    track('perodua_model_selected', { model: modelId });
  };
      ...
      <div className="container mx-auto px-4">
        <ScrollReveal>
          ...
          <p className="text-gray-600 text-center mb-8">
            {language === 'ms' 
              ? 'Imbas alat ganti atau manual Perodua untuk mendapatkan maklumat terperinci' 
              : 'Scan Perodua parts or manuals to get detailed information'}
          </p>
        </ScrollReveal>
        ...
        <div className="max-w-3xl mx-auto">
          {!selectedModel ? (
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <h2 className="text-xl font-semibold mb-4">
                {language === 'ms' ? 'Pilih Model Perodua' : 'Select Perodua Model'}
              </h2>
          ...
          <div className="mt-8 bg-blue-50 rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-2">
              {language === 'ms' ? 'Tentang Pengimbas Alat Ganti Perodua' : 'About Perodua Parts Scanner'}
            </h2>
            <p className="text-gray-700 mb-4">
              {language === 'ms' 
                ? 'Pengimbas ini direka untuk mengenal pasti alat ganti Perodua Malaysia berdasarkan imej atau manual. Ia menyokong model-model berikut:' 
                : 'This scanner is designed to identify Perodua Malaysia parts based on images or manuals. It supports the following models:'}
            </p>
            ...
            <p className="text-sm text-gray-600">
              {language === 'ms' 
                ? 'Data alat ganti diambil dari katalog rasmi Perodua Malaysia.' 
                : 'Parts data is sourced from the official Perodua Malaysia catalog.'}
            </p>
