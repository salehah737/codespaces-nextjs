...
import { yamahaModels, yamahaCommonParts, findPartsByImage } from '../../lib/yamaha-parts-data';

export default function YamahaPartScanner({ onScanResult }) {
  ...
  const captureFrame = () => {
    ...
    processYamahaImage(imageData);
  };

  // Process Yamaha image to find parts
  const processYamahaImage = (imageData) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (selectedModel) {
          ...
          const model = yamahaModels.find(m => m.id === selectedModel);
          parts = model ? model.parts : [];
          
          // Add compatible common parts
          const compatibleCommonParts = yamahaCommonParts.filter(part => 
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      {scanResult && scanResult.status === 'success' && (
        <div className="mt-4">
          <h3 className="font-medium text-lg mb-2">
            {language === 'ms' ? 'Alat Ganti Yamaha Dikesan' : 'Yamaha Parts Detected'}
          </h3>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          {language === 'ms' 
            ? 'Data alat ganti dari katalog Yamaha Malaysia.' 
            : 'Parts data from Yamaha Malaysia catalog.'}
        </p>
        <p className="mt-1">
          <a 
          ...
          >
            {language === 'ms' ? 'Sumber: yamaha-motor.com.my' : 'Source: yamaha-motor.com.my'}
          </a>
        </p>
      </div>
    </div>
