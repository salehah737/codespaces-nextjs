...
export default function HondaPartScanner({ onScanResult, preselectedModel }) {
  ...
  const captureFrame = () => {
    ...
    processHondaImage(imageData);
  };

  // Process Honda image to find parts
  const processHondaImage = (imageData) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (selectedModel) {
          ...
          const model = hondaModels.find(m => m.id === selectedModel);
          if (model) {
            parts = [...model.parts];
            
            // Add compatible common parts
            const compatibleCommonParts = hondaCommonParts.filter(part => 
              part.compatibility.includes(model.name)
            );
            parts = [...parts, ...compatibleCommonParts];
          }
        } else {
          // Otherwise show sample parts from the first model
          parts = hondaModels[0].parts.slice(0, 1);
          if (hondaCommonParts.length > 0) {
            parts.push(hondaCommonParts[0]);
          }
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      {scanResult && scanResult.status === 'success' && (
        <div className="mt-4">
          <h3 className="font-medium text-lg mb-2">
            {language === 'ms' ? 'Alat Ganti Honda Dikesan' : 'Honda Parts Detected'}
          </h3>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          {language === 'ms' 
            ? 'Data alat ganti dari katalog Honda Malaysia.' 
            : 'Parts data from Honda Malaysia catalog.'}
        </p>
        <p className="mt-1">
          <a 
          ...
          >
            {language === 'ms' ? 'Sumber: honda.com.my' : 'Source: honda.com.my'}
          </a>
        </p>
      </div>
    </div>
