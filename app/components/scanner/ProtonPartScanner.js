...
export default function ProtonPartScanner({ onScanResult, preselectedModel }) {
  ...
  const captureFrame = () => {
    ...
    processProtonImage(imageData);
  };

  // Process Proton image to find parts
  const processProtonImage = (imageData) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (selectedModel) {
          ...
          const model = protonModels.find(m => m.id === selectedModel);
          if (model) {
            parts = [...model.parts];
            
            // Add compatible common parts
            const compatibleCommonParts = protonCommonParts.filter(part => 
          ...
          parts = protonModels[0].parts.slice(0, 1);
          if (protonCommonParts.length > 0) {
            parts.push(protonCommonParts[0]);
          }
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      ...
      {scanResult && scanResult.status === 'success' && (
        <div className="mt-4">
          <h3 className="font-medium text-lg mb-2">
            {language === 'ms' ? 'Alat Ganti Proton Dikesan' : 'Proton Parts Detected'}
          </h3>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          {language === 'ms' 
            ? 'Data alat ganti dari katalog Proton Malaysia.' 
            : 'Parts data from Proton Malaysia catalog.'}
        </p>
        <p className="mt-1">
          <a 
          ...
          >
            {language === 'ms' ? 'Sumber: proton.com' : 'Source: proton.com'}
          </a>
        </p>
      </div>
    </div>
